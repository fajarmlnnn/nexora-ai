...
class _AIGlowPageState extends ConsumerState<AIGlowPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> ask(String q) async {
    final t = q.trim();
    final n = norm(q);
    if (t.isEmpty || sending || (last == n && lastAt != null && DateTime.now().difference(lastAt!) < const Duration(seconds: 3))) return;
  ...
  Widget header() => Padding(
    ...
    child: Row(children: [
      ...
      const H(icon: LucideIcons.history, label: 'Riwayat percakapan'),
      const SizedBox(width: 6),
      H(icon: LucideIcons.squarePen, label: 'Percakapan baru', onTap: newChat),
    ]),
  );
        ...
        child: Stack(clipBehavior: Clip.none, children: [
          ...
          Positioned(
            ...
            child: AnimatedBuilder(
              animation: anim,
              builder: (_, child) => Transform.translate(offset: Offset(0, -2 * anim.value), child: child),
              child: Image.asset('assets/mascot/nexora-analytic.png', fit: BoxFit.contain, filterQuality: FilterQuality.high),
            ),
          ),
          Align(
            ...
            child: ConstrainedBox(
              ...
              child: Padding(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  RichText(text: TextSpan(style: AppTypography.heading1.copyWith(fontSize: 28, height: 1.05), children: const [
                    TextSpan(text: 'Aku '),
  ...
  Widget features() => Padding(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Padding(padding: const EdgeInsets.only(right: 16), child: Row(children: [
        Text('Mulai dari sini', style: AppTypography.heading2.copyWith(fontSize: 25)),
        const Spacer(),
  ...
  Widget composer() => SafeArea(
    ...
    child: Padding(
      ...
      child: Container(
        constraints: const BoxConstraints(minHeight: 62, maxHeight: 72),
        padding: const EdgeInsets.all(5),
        decoration: BoxDecoration(
          color: AppColors.inputBar,
          borderRadius: BorderRadius.circular(22),
          border: Border.all(color: AppColors.primary.withValues(alpha: .38)),
          boxShadow: [BoxShadow(color: AppColors.primary.withValues(alpha: .12), blurRadius: 18, offset: const Offset(0, 5))],
        ),
        child: Row(children: [
          ...
          Expanded(child: TextField(
            ...
            onSubmitted: (v) { if (!v.contains('\n')) ask(v); },
            style: AppTypography.bodySmall.copyWith(color: Colors.white),
            decoration: InputDecoration(hintText: sending ? 'Nexora sedang berpikir...' : 'Tanya apa saja tentang keuanganmu...', border: InputBorder.none, isDense: true, contentPadding: const EdgeInsets.symmetric(vertical: 7), hintStyle: AppTypography.bodySmall.copyWith(color: AppColors.textMuted)),
          )),
          const SizedBox(width: 8),
          InkWell(
            onTap: sending ? null : () => ask(c.text),
            borderRadius: BorderRadius.circular(18),
            child: Ink(width: 42, height: 42, decoration: BoxDecoration(shape: BoxShape.circle, color: sending ? AppColors.cardMuted : AppColors.sendButton), child: Center(child: sending ? const SizedBox(width: 15, height: 15, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(LucideIcons.arrowUpRight, color: Colors.white, size: 20))),
          ),
        ]),
      ),
    ),
