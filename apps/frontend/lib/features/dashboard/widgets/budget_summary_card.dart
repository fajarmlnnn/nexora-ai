    ...
    return NCard(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(child: Text('Budget Summary', style: AppTypography.labelLarge.copyWith(fontWeight: FontWeight.w700))),
              TextButton(
                onPressed: () => context.push('/budget'),
                style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: const Size(0, 0), tapTargetSize: MaterialTapTargetSize.shrinkWrap),
                child: Text('Lihat Semua  ›', style: AppTypography.caption.copyWith(color: AppColors.primaryLight, fontWeight: FontWeight.w700, fontSize: 10)),
              ),
            ],
          ),
          if (items.isEmpty)
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: _Metric(label: isOver ? 'Over' : 'Sisa', value: rupiah(remaining.abs()), valueColor: isOver ? AppColors.danger : AppColors.success)),
              const SizedBox(width: 8),
            ...
            ClipRRect(
              ...
              child: LinearProgressIndicator(value: progress, minHeight: 3, backgroundColor: Colors.white.withValues(alpha: .06), valueColor: AlwaysStoppedAnimation<Color>(isOver ? AppColors.danger : AppColors.primary)),
            ),
            if (isOver) ...[
              const SizedBox(height: 6),
              Text('OVER BUDGET • Melebihi ${rupiah(remaining.abs())}', style: AppTypography.caption.copyWith(color: AppColors.danger, fontWeight: FontWeight.w800, fontSize: 9)),
            ],
            if (visibleItems.isNotEmpty) ...[
              const SizedBox(height: 8),
              for (int i = 0; i < visibleItems.length; i++) ...[
                _BudgetRow(item: visibleItems[i]),
                if (i != visibleItems.length - 1) Divider(height: 12, color: Colors.white.withValues(alpha: .055)),
              ],
            ],
          ],
        ],
...
class _Metric extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerLeft, child: Text(value, maxLines: 1, softWrap: false, style: AppTypography.currency.copyWith(fontSize: 12.5, fontWeight: FontWeight.w800, color: valueColor ?? AppColors.textPrimary))),
      ]);
}
...
class _ProgressRing extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SizedBox(width: 38, height: 38, child: Stack(alignment: Alignment.center, children: [
      CircularProgressIndicator(value: value, strokeWidth: 3.5, backgroundColor: Colors.white.withValues(alpha: .06), valueColor: AlwaysStoppedAnimation<Color>(color)),
      Text(isOver ? 'OVER' : '$percentage%', style: AppTypography.caption.copyWith(color: color, fontWeight: FontWeight.w800, fontSize: isOver ? 7 : 8)),
    ]));
  }
}
...
class _BudgetRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Row(children: [
      Container(width: 32, height: 32, decoration: BoxDecoration(color: item.color.withValues(alpha: .08), borderRadius: BorderRadius.circular(9), border: Border.all(color: item.color.withValues(alpha: .10))), child: Icon(_icon(item.id), size: 15, color: item.color.withValues(alpha: .9))),
      const SizedBox(width: 9),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(child: Text(item.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.labelMedium.copyWith(fontWeight: FontWeight.w700, fontSize: 10.5))),
        ...
        const SizedBox(height: 3),
        ClipRRect(borderRadius: BorderRadius.circular(999), child: LinearProgressIndicator(value: item.progress, minHeight: 3, backgroundColor: Colors.white.withValues(alpha: .06), valueColor: AlwaysStoppedAnimation<Color>(accent))),
      ])),
      const SizedBox(width: 9),
