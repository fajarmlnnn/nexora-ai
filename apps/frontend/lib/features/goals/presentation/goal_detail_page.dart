...
class GoalDetailPage extends ConsumerWidget {
            ...
            Row(
              children: [
                ...
                PopupMenuButton<String>(
                  onSelected: (value) => _menu(context, ref, goal, value),
                  itemBuilder: (_) => const [
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            Text('${(goal.progress * 100).round()}%', style: AppTypography.heading1.copyWith(fontSize: 26)),
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            completed ? 'Target tercapai. Lanjutkan goal yang sama dengan menaikkan target.' : 'Sisa target: ${rupiah(goal.remaining)}',
  ...
  Future<void> _menu(BuildContext context, WidgetRef ref, FinancialGoalSnapshot goal, String value) async {
    if (value == 'increase') {
      await _increase(context, ref, goal, (goal.target * .25).clamp(100000.0, double.infinity));
      return;
    }
    ...
    if (value == 'rename') {
      ...
      if (title != null && title.isNotEmpty) {
        await ref.read(financialGoalsProvider.notifier).updateGoal(goal.id, title: title);
      }
      return;
    }
    ...
    if (value == 'pause') {
      ...
      if (context.mounted && ok) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(paused ? 'Goal dilanjutkan.' : 'Goal dijeda.')));
      }
      return;
    }
    ...
    if (value == 'delete') {
      final confirmed = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text(
            goal.saved > 0
                ? 'Goal "${goal.title}" akan dihapus dan seluruh dana yang sudah disetorkan (${rupiah(goal.saved)}) akan dikembalikan ke wallet. Histori transaksi setoran goal juga dibatalkan.'
                : 'Goal "${goal.title}" akan dihapus. Goal ini belum memiliki dana yang harus dikembalikan.',
          ),
          actions: [
      ...
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Goal dihapus dan dana dikembalikan ke wallet.')));
          Navigator.pop(context);
        }
      } catch (error) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Goal tidak dihapus: $error')));
        }
      }
    }
  }
  ...
  Future<void> _increase(BuildContext context, WidgetRef ref, FinancialGoalSnapshot goal, double suggested) async {
    ...
    final amount = await showDialog<double>(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text('Batal')),
          FilledButton(onPressed: () => Navigator.pop(context, double.tryParse(controller.text.replaceAll('.', '').replaceAll(',', ''))), child: const Text('Tambah')),
        ],
      ),
    );
    controller.dispose();
...
class _AI extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(gradient: AppGradients.primary, borderRadius: AppRadius.radiusXL, boxShadow: AppShadows.glow),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Wrap(
            ...
            runSpacing: 7,
            children: [
              OutlinedButton.icon(onPressed: onTopUp, icon: const Icon(LucideIcons.plus, size: 15), label: Text(completed ? 'Naikkan Target' : 'Tambah Dana'), style: OutlinedButton.styleFrom(foregroundColor: Colors.white, side: BorderSide(color: Colors.white.withValues(alpha: .45)))),
              if (completed) OutlinedButton.icon(onPressed: onIncrease, icon: const Icon(LucideIcons.arrowUpRight, size: 15), label: const Text('Target Baru'), style: OutlinedButton.styleFrom(foregroundColor: Colors.white, side: BorderSide(color: Colors.white.withValues(alpha: .45)))),
            ],
          ),
        ],
      ),
