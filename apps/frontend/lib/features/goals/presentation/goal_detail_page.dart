...
class GoalDetailPage extends ConsumerWidget {
        ...
        child: Column(
          ...
          children: [
            ...
            PremiumCard(
              ...
              child: Column(
                children: [
                  ...
                  Text(
                    'dari target ${rupiah(goal.target)}',
                  ...
                  Container(
                    ...
                    child: Row(
                      children: [
                        Icon(
                          completed ? LucideIcons.badgeCheck : LucideIcons.flag,
                          size: 18,
                          color: completed
                              ? AppColors.success
                        ...
                        Expanded(
                          child: Text(
                            completed
                                ? 'Target tercapai. Lanjutkan goal yang sama dengan menaikkan target.'
            ...
            _AI(
              ...
              completed: completed,
              suggested: suggested,
              onTopUp: () => _contribute(context, ref, goal, suggested),
            ...
            Row(
              children: [
                Expanded(
                  child: _Stat('Tersimpan', rupiah(goal.saved), LucideIcons.walletCards),
                ),
                const SizedBox(width: 7),
                Expanded(
                  child: _Stat('Sisa', rupiah(goal.remaining), LucideIcons.flag),
                ),
                const SizedBox(width: 7),
                Expanded(
                  child: _Stat(
                    '/ bulan',
                    rupiah(goal.suggestedMonthlyContribution),
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (goal.deadline != null) ...[
                    ...
                    Text(
                      'Deadline: ${goal.deadline!.day}/${goal.deadline!.month}/${goal.deadline!.year} • ${goal.daysRemaining < 0 ? 'melewati deadline' : '${goal.daysRemaining} hari lagi'}',
                      style: AppTypography.caption,
                    ),
                  ],
                ],
    ...
    if (value == 'pause') {
      ...
      if (context.mounted && ok) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(paused ? 'Goal dilanjutkan.' : 'Goal dijeda.')),
        );
      }
      return;
    }
    ...
    if (value == 'delete') {
      final confirmed = await showDialog<bool>(
        ...
        builder: (_) => AlertDialog(
          ...
          content: Text(
            'Goal "${goal.title}" dan histori kontribusinya akan dihapus.',
          ),
          actions: [
            TextButton(
  ...
  Future<void> _contribute(
    ...
    if (amount == null || amount <= 0) return;
    final ok = await ref.read(financialGoalsProvider.notifier).contribute(
          goal.id,
          amount,
        );
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            ok ? 'Progress goal diperbarui.' : 'Kontribusi gagal disimpan.',
          ),
        ),
      );
    }
