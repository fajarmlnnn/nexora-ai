...
class _GoalsPageState extends ConsumerState<GoalsPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final tabs = <List<FinancialGoalSnapshot>>[
      allGoals,
      allGoals.where((g) => g.type == 'Wishlist').toList(),
      allGoals.where((g) => g.type == 'Saving').toList(),
      allGoals.where((g) => g.type == 'Debt').toList(),
    ];
        ...
        Material(
          ...
          child: InkWell(
            ...
            child: Ink(
              ...
              child: const Icon(LucideIcons.plus, size: 27, color: Colors.white),
            ),
          ),
        ),
      ],
      ...
      child: Column(
        children: [
          Row(
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    Icon(LucideIcons.flag, size: 17, color: AppColors.primaryLight),
                    const SizedBox(height: 2),
                    Text('${goals.length}', style: AppTypography.labelLarge.copyWith(fontWeight: FontWeight.w800)),
          ...
          Row(
            children: [
              Expanded(
                child: Text('Sisa ${rupiah(remaining)} lagi', style: AppTypography.caption),
              ),
              Text(
                '${(progress * 100).round()}%',
                style: AppTypography.caption.copyWith(
          ...
          (index) => Expanded(
            child: GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                alignment: Alignment.center,
                decoration: BoxDecoration(
                  gradient: selected == index ? AppGradients.primary : null,
                  borderRadius: AppRadius.radiusMD,
                  boxShadow: selected == index ? AppShadows.glow : null,
                ),
                child: Text(
                  labels[index],
                  style: AppTypography.caption.copyWith(
                    color: selected == index ? Colors.white : AppColors.textSecondary,
                    fontWeight: FontWeight.w800,
                  ),
                ),
              ),
      ...
      return ListView(
        ...
        children: [
          ...
          Center(
            child: Column(
              children: [
                Icon(LucideIcons.target, size: 44, color: AppColors.primaryLight.withValues(alpha: .75)),
                const SizedBox(height: 12),
                Text('Belum ada goal', style: AppTypography.labelLarge.copyWith(fontWeight: FontWeight.w800)),
                const SizedBox(height: 4),
                Text('Buat target pertama kamu.', style: AppTypography.caption),
              ],
            ),
          ),
          if (showInsight) ...[
            const SizedBox(height: 22),
            const ContextAIInsight(message: 'Butuh bantuan mencapai goals? Nexora AI bisa membantu membuat rencana target dan rekomendasi langkah berikutnya.'),
          ],
        ],
      );
    }
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        if (showSectionHeader && index == 0) {
          return _SectionHeader(sortByProgress: sortByProgress, onToggleSort: onToggleSort);
        }
        final goalIndex = index - (showSectionHeader ? 1 : 0);
        if (showInsight && goalIndex == goals.length) {
          return const ContextAIInsight(
            message: 'Butuh bantuan mencapai goals? AI akan menganalisis kebiasaanmu dan membuat rekomendasi khusus untukmu.',
          );
        }
...
class _SectionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        Text('Goals aktif', style: AppTypography.labelLarge.copyWith(fontWeight: FontWeight.w800)),
        const Spacer(),
        TextButton.icon(
          onPressed: onToggleSort,
          style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: Size.zero, tapTargetSize: MaterialTapTargetSize.shrinkWrap),
          icon: Icon(LucideIcons.slidersHorizontal, size: 17, color: AppColors.textSecondary),
          label: Text(
          ...
          child: Row(
            ...
            children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Text(
                            goal.type,
                            style: AppTypography.overline.copyWith(color: accent, fontWeight: FontWeight.w800),
                          ),
                        ),
                      ],
                    ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: AppTypography.overline.copyWith(fontWeight: FontWeight.w700),
                    ),
                    const SizedBox(height: 8),
                    Row(
                      mainAxisAlignment: MainAxisAlignment.end,
                      children: [
                        Icon(LucideIcons.calendarDays, size: 13, color: AppColors.textMuted),
              ...
              Padding(
                ...
                child: Icon(LucideIcons.moreVertical, size: 18, color: AppColors.textMuted),
              ),
            ],
          ),
        ),
