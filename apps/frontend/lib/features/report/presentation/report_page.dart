...
class ReportPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final incomeChange = previous.income <= 0
        ? 0.0
        : ((analytics.income - previous.income) / previous.income) * 100;
    final expenseChange = previous.expense <= 0
        ? 0.0
        : ((analytics.expense - previous.expense) / previous.expense) * 100;
        ...
        children: [
          Row(
            children: [
              ...
              _MonthSelector(label: _monthLabel(analytics.start)),
            ],
          ),
          AppSpacing.gapLG,
          Row(
            children: [
              ...
              Expanded(
                child: MetricPill(
                  ...
                  label: 'Expense',
                  value: rupiah(analytics.expense),
            ...
            children: [
              Expanded(
                child: MetricPill(
                  ...
                  label: 'Savings Rate',
                  value: '${(analytics.savingsRate * 100).toStringAsFixed(1)}%',
              ...
              Expanded(
                child: MetricPill(
                  ...
                  label: 'Net Cashflow',
                  value: '${analytics.netCashflow >= 0 ? '+' : '-'}${rupiah(analytics.netCashflow.abs())}',
                ...
                if (topCategories.isEmpty)
                  ...
                  ...topCategories.take(3).map(
                    (entry) => _SpendingRow(
                      ...
                      value: rupiah(entry.value),
                      progress: maxCategory <= 0 ? 0 : entry.value / maxCategory,
                ...
                Expanded(
                  child: Text(
                    analytics.transactionCount == 0
                        ? 'Belum ada transaksi bulan ini. Mulai mencatat transaksi agar laporan keuangan semakin akurat.'
...
class _ComparisonRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final changeColor = invertChangeColor
        ? (positive ? AppColors.danger : AppColors.success)
      ...
      child: Row(
        children: [
          ...
          Text(
            ...
            style: AppTypography.caption.copyWith(color: changeColor),
          ),
        ],
      ),
    );
    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          AnimatedProgressBar(value: percent, color: color),
        ],
      ),
    );
  }
...
String _reportInsight(FinancialAnalyticsSnapshot analytics) {
  ...
  if (top == null || analytics.expense <= 0) {
    return 'Cashflow bulan ini masih positif. Pertahankan pencatatan transaksi agar insight semakin akurat.';
  }
  final percent = top.value / analytics.expense * 100;
  return 'Pengeluaran terbesar bulan ini adalah ${_categoryLabel(top.key)} sebesar ${rupiah(top.value)} (${percent.toStringAsFixed(1)}% dari total pengeluaran).';
}
