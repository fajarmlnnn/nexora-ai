...
class _TransactionPageState extends ConsumerState<TransactionPage> {
  ...
  Future<void> _editTransaction(TransactionModel item) async {
    if (item.isTransfer) {
      _showMessage(
        'Transfer belum bisa diedit. Hapus lalu buat transfer baru jika diperlukan.',
      );
      return;
    }
              ...
              child: transactions.when(
                loading: () => ListView.separated(
                  ...
                  itemBuilder: (context, index) =>
                      const ShimmerSkeleton(height: 70),
                ),
                ...
                data: (items) {
                  final visible = items
                      .where(
                        (item) => filter == null || item.type == filter,
                      )
                      .where((item) {
...
class _FilterTabs extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final labels = ['Semua', 'Masuk', 'Keluar', 'Transfer'];
    final index = _index();
...
class _Group extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(
        ...
        children: [
          ...
          for (final item in items)
            _TransactionTile(item: item, onTap: onTap),
...
class _TransactionDetailSheet extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final source = item.sourceAccount?.trim() ?? '';
    final destination = item.destinationAccount?.trim() ?? '';
          ...
          child: Column(
            ...
            children: [
              ...
              _DetailRow(
                ...
                label: 'Tanggal',
                value: DateFormat('dd MMMM yyyy', 'id_ID').format(item.date),
              ),
              _DetailRow(
                icon: LucideIcons.clock3,
                label: 'Waktu',
                value: '${DateFormat('HH:mm').format(item.date)} WIB',
              ),
              _DetailRow(
              ...
              if (note.isNotEmpty) ...[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: AppColors.card.withValues(alpha: .75),
                    borderRadius: AppRadius.radiusLG,
                    border: Border.all(
                      color: AppColors.border.withValues(alpha: .35),
                    ),
...
IconData _icon(TransactionCategory category) => switch (category) {
