        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Expanded(
            child: transactions.when(
              loading: () => ListView.separated(controller: _transactionScrollController, padding: EdgeInsets.only(top: 8, bottom: AppSpacing.bottomNav(context) + 28), itemCount: 6, separatorBuilder: (_, _) => const SizedBox(height: 8), itemBuilder: (_, _) => const ShimmerSkeleton(height: 68)),
              ...
              data: (items) {
                ...
                final expanded = [...items, ..._extraTransactions()]
                    ...
                    .where((item) => query.isEmpty || item.title.toLowerCase().contains(query) || _categoryLabel(item.category).toLowerCase().contains(query) || (item.isTransfer && 'transfer'.contains(query)))
                    .toList();
                ...
                return ListView(
                  ...
                  padding: EdgeInsets.only(top: 8, bottom: AppSpacing.bottomNav(context) + 28),
                  children: [
...
class _TransactionHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Transaksi', style: AppTypography.heading1), const SizedBox(height: 1), Text('Pantau arus uangmu', style: AppTypography.bodySmall)])),
    Material(color: AppColors.card.withValues(alpha: .72), shape: const CircleBorder(), child: InkWell(onTap: onFilter, customBorder: const CircleBorder(), child: const SizedBox(width: 44, height: 44, child: Icon(LucideIcons.slidersHorizontal, size: 20, color: AppColors.textSecondary)))),
  ]);
}
...
class _SearchBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(children: [
      const Icon(LucideIcons.search, color: AppColors.textMuted, size: 20), const SizedBox(width: 9),
      ...
      const SizedBox(width: 5), GestureDetector(onTap: onFilter, child: const Icon(LucideIcons.listFilter, color: AppColors.primaryLight, size: 19)),
    ]),
  );
}
...
class _TransactionTileState extends State<_TransactionTile> {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: ClipRRect(borderRadius: AppRadius.radiusXL, child: Stack(alignment: Alignment.centerRight, children: [
        ...
        AnimatedContainer(duration: _settleDuration, curve: Curves.easeOutCubic, transform: Matrix4.translationValues(_offset, 0, 0), transformAlignment: Alignment.center, child: GestureDetector(
          behavior: HitTestBehavior.opaque,
          onHorizontalDragUpdate: _dragUpdate,
          onHorizontalDragEnd: _dragEnd,
          onTap: () { if (_open) setState(() { _open = false; _offset = 0; }); },
          child: PremiumCard(borderRadius: AppRadius.radiusXL, padding: const EdgeInsets.symmetric(horizontal: 11, vertical: 10), child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
              Text(item.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.labelMedium.copyWith(fontWeight: FontWeight.w700)), const SizedBox(height: 1),
