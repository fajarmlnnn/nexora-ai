    ...
    return PremiumScaffold(
      child: SafeArea(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            SliverPadding(padding: AppSpacing.screen.copyWith(bottom: 0), sliver: SliverToBoxAdapter(child: _Header(onAdd: () => Navigator.push(context, MaterialPageRoute<void>(builder: (_) => const MoneyFormPage(income: false))))),
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (_) => _Detail(
        ...
        onEdit: () {
          ...
          if (!item.isTransfer) Navigator.push(context, MaterialPageRoute<void>(builder: (_) => MoneyFormPage(income: item.isIncome, transaction: item)));
        },
        onDelete: () async {
          Navigator.pop(context);
          final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(backgroundColor: AppColors.card, title: const Text('Hapus transaksi?'), content: Text('Hapus ${item.title} sebesar ${rupiah(item.amount)}?'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Batal')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Hapus'))]));
          if (ok == true) {
...
class _Filters extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final data = <({String label, TransactionType? type})>[
      (label: 'Semua', type: null),
      (label: 'Masuk', type: TransactionType.income),
      (label: 'Keluar', type: TransactionType.expense),
      (label: 'Transfer', type: TransactionType.transfer),
    ];
    return SizedBox(height: 42, child: ListView.separated(scrollDirection: Axis.horizontal, itemCount: data.length, separatorBuilder: (_, __) => const SizedBox(width: 8), itemBuilder: (_, i) {
      final x = data[i];
      final active = selected == x.type;
      return ChoiceChip(label: Text(x.label), selected: active, onSelected: (_) => onChanged(x.type), selectedColor: AppColors.primary.withValues(alpha: .25), backgroundColor: AppColors.card.withValues(alpha: .7), side: BorderSide(color: active ? AppColors.primaryLight.withValues(alpha: .55) : AppColors.border.withValues(alpha: .25)), labelStyle: TextStyle(color: active ? Colors.white : AppColors.textSecondary, fontWeight: FontWeight.w800));
    }));
  }
}
...
class _Metric extends StatelessWidget {
  const _Metric({required this.label, required this.value, required this.positive});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (item.isTransfer) ...[_Info(label: 'Dari', value: item.sourceAccount ?? '-'), _Info(label: 'Ke', value: item.destinationAccount ?? '-')],
