...
class _TransferWalletSheetState extends ConsumerState<_TransferWalletSheet> {
  ...
  Future<void> _submit(List<WalletModel> wallets) async {
    ...
    final destinationWallet = destination;

    if (sourceWallet.isHidden || destinationWallet.isHidden) {
      _showError('Wallet tersembunyi tidak dapat digunakan untuk transfer.');
      return;
    }
    if (sourceWallet.balance < amount) {
      _showError('Saldo ${sourceWallet.name} tidak mencukupi.');
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('${rupiah(amount)} berhasil dipindahkan ke ${destinationWallet.name}.'),
  ...
  Widget build(BuildContext context) {
    ...
    if (wallets.length < 2) {
      return Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(
            'Minimal dua wallet aktif diperlukan untuk melakukan transfer.',
    ...
    _destinationId ??= wallets.firstWhere(
      (wallet) => wallet.id != _sourceId,
      orElse: () => wallets.last,
    ).id;
      ...
      child: Form(
        ...
        child: SingleChildScrollView(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(
              'Memindahkan dana tanpa mengubah total aset atau cashflow.',
              style: AppTypography.bodySmall,
            ),
            const SizedBox(height: 18),
            _walletDropdown(
            ...
            TextFormField(
              ...
              decoration: _decoration(
                ...
                hint: 'Contoh: pindah dana operasional',
              ),
            ),
