...
class _TransferWalletSheetState extends ConsumerState<_TransferWalletSheet> {
  ...
  Future<void> _submit(List<WalletModel> wallets) async {
    ...
    final source = wallets.where((wallet) => wallet.id == sourceId).firstOrNull;
    final destination = wallets.where((wallet) => wallet.id == destinationId).firstOrNull;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('${rupiah(amount)} berhasil dipindahkan ke ${destination.name}.'),
        ...
        child: SingleChildScrollView(
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _walletDropdown(
              ...
              value: _sourceId,
              wallets: wallets,
              onChanged: _saving ? null : (value) => setState(() => _sourceId = value),
            ),
            const SizedBox(height: 12),
            ...
            _walletDropdown(
              ...
              wallets: wallets,
              onChanged: _saving ? null : (value) => setState(() => _destinationId = value),
            ),
            const SizedBox(height: 12),
            TextFormField(
              ...
              validator: (value) {
                final amount = double.tryParse((value ?? '').replaceAll('.', '').replaceAll(',', '').trim());
                if (amount == null || amount <= 0) return 'Masukkan nominal transfer yang valid.';
                return null;
              },
              decoration: _decoration('Nominal transfer', LucideIcons.banknote, hint: '0'),
            ),
            const SizedBox(height: 12),
            TextFormField(
              ...
              maxLines: 2,
              style: AppTypography.bodySmall.copyWith(color: Colors.white),
              decoration: _decoration('Catatan (opsional)', LucideIcons.notebookPen, hint: 'Contoh: pindah dana operasional'),
            ),
            ...
            SizedBox(
              ...
              child: DecoratedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: _saving ? null : () => _submit(wallets),
                  icon: _saving ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white)) : const Icon(LucideIcons.arrowLeftRight),
                  label: Text(_saving ? 'Memproses...' : 'Transfer Sekarang'),
  ...
  Widget _walletDropdown({required String label, required String? value, required List<WalletModel> wallets, required ValueChanged<String?>? onChanged}) {
    return DropdownButtonFormField<String>(
      initialValue: value,
      decoration: _decoration(label, LucideIcons.walletMinimal),
      dropdownColor: AppColors.card,
      items: wallets.map((wallet) => DropdownMenuItem(value: wallet.id, child: Text('${wallet.name} • ${rupiah(wallet.balance)}', overflow: TextOverflow.ellipsis))).toList(),
      onChanged: onChanged,
    );
  }
