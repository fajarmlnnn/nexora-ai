...
class WalletDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final related = transactions.where((tx) {
      return tx.walletId == wallet.id || tx.sourceAccount == wallet.id || tx.destinationAccount == wallet.id;
    }).toList()
            ...
            PremiumCard(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  ...
                  if (wallet.isPrimary) _StatusChip(label: 'PRIMARY', color: AppColors.primaryLight),
                ]),
  ...
  Future<void> _confirmDelete(BuildContext context, WidgetRef ref, WalletModel wallet) async {
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(title: const Text('Hapus wallet?'), content: Text('Wallet ${wallet.name} akan dihapus dari daftar wallet.'), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Batal')), FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Hapus'))]));
    if (confirmed != true || !context.mounted) return;
    final success = await ref.read(walletProvider.notifier).deleteWallet(wallet.id);
    if (!context.mounted) return;
    if (success) Navigator.pop(context);
    else ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Wallet gagal dihapus.')));
  }
}
...
class _AiCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF17131F), Color(0xFF0D0D14)]), borderRadius: AppRadius.radiusXL, border: Border.all(color: AppColors.primary.withValues(alpha: .24))),
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(positive ? 'Cashflow wallet ini positif.' : 'Cashflow wallet ini sedang negatif.', style: AppTypography.bodySmall.copyWith(fontWeight: FontWeight.w700)),
          const SizedBox(height: 3),
          Text(positive ? 'Pemasukan lebih besar daripada pengeluaran pada data transaksi yang tersedia.' : 'Pengeluaran lebih besar daripada pemasukan pada data transaksi yang tersedia.', style: AppTypography.caption),
        ])),
      ]),
    );
  }
...
class _FlowRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Padding(padding: const EdgeInsets.symmetric(vertical: 4), child: Row(children: [Expanded(child: Text(label, style: AppTypography.caption)), Text(rupiah(amount), style: AppTypography.labelSmall.copyWith(color: color, fontWeight: FontWeight.w800))]));
}
...
class _TransactionRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final sign = transaction.isTransfer ? (incoming ? '+' : '-') : (incoming ? '+' : '-');
    return Padding(padding: const EdgeInsets.only(bottom: 7), child: PremiumCard(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), child: Row(children: [Container(width: 34, height: 34, decoration: BoxDecoration(color: color.withValues(alpha: .11), borderRadius: AppRadius.radiusLG), child: Icon(transaction.isTransfer ? LucideIcons.arrowLeftRight : (incoming ? LucideIcons.arrowDownLeft : LucideIcons.arrowUpRight), size: 17, color: color)), const SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(transaction.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.labelMedium.copyWith(fontWeight: FontWeight.w700)), Text('${transaction.date.day.toString().padLeft(2, '0')}/${transaction.date.month.toString().padLeft(2, '0')}/${transaction.date.year}', style: AppTypography.caption)])), Text('$sign${rupiah(transaction.amount)}', style: AppTypography.labelSmall.copyWith(color: color, fontWeight: FontWeight.w800))])));
  }
}
