...
class _WalletDetailPageState extends ConsumerState<WalletDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final wallet = wallets.where((item) => item.id == widget.walletId).firstOrNull;
      ...
      child: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            ...
            const SizedBox(height: 18),
            _SectionTitle(title: 'Informasi Wallet'),
            const SizedBox(height: 9),
            _InfoCard(wallet: wallet),
            const SizedBox(height: 18),
            _SectionTitle(title: 'Nexora AI'),
            const SizedBox(height: 9),
...
class _StatsGrid extends StatelessWidget {
  const _StatsGrid({required this.wallet});
  final WalletModel wallet;
  @override
  Widget build(BuildContext context) => Row(children: [
...
class _InfoCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => PremiumCard(padding: const EdgeInsets.symmetric(vertical: 5), borderRadius: AppRadius.radiusXL, child: Column(children: [
        ...
        _InfoRow(label: 'Penyedia', value: wallet.bankName),
        _InfoRow(label: 'Nomor akun', value: wallet.maskedAccount),
        _InfoRow(label: 'ID wallet', value: wallet.id),
      ]));
}
...
class _AiCard extends StatelessWidget {
  const _AiCard({required this.wallet});
  final WalletModel wallet;
  @override
  Widget build(BuildContext context) {
    final advice = wallet.balance <= 0 ? 'Saldo wallet ini kosong. Pertimbangkan mengisinya hanya jika memang punya fungsi yang jelas.' : wallet.isPrimary ? 'Wallet utama sebaiknya menyimpan dana operasional, bukan seluruh aset. Pisahkan dana tujuan agar arus kas lebih mudah dikontrol.' : 'Wallet ini bisa dipakai sebagai kantong khusus agar distribusi aset lebih mudah dipantau.';
...
class _DangerActions extends StatelessWidget {
  const _DangerActions({required this.wallet});
  final WalletModel wallet;
  @override
  Widget build(BuildContext context) => PremiumCard(padding: EdgeInsets.zero, borderRadius: AppRadius.radiusXL, child: Column(children: [
        ...
        ListTile(leading: const Icon(LucideIcons.trash2, size: 19, color: AppColors.error), title: const Text('Hapus wallet'), subtitle: const Text('Pastikan tidak ada data penting yang bergantung padanya'), onTap: () => _confirmDelete(context)),
      ]));

  Future<void> _confirmDelete(BuildContext context) async {
    final confirmed = await showDialog<bool>(context: context, builder: (dialogContext) => AlertDialog(title: const Text('Hapus wallet?'), content: const Text('Wallet akan dihapus dari daftar. Riwayat transaksi tidak ikut dihapus.'), actions: [TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Batal')), FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: const Text('Hapus'))]));
