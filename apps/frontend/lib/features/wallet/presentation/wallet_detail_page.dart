...
class WalletDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final wallet = walletState.valueOrNull?.cast<WalletModel?>().firstWhere(
    ...
    final allTransactions = ref.watch(financialTransactionStoreProvider);
    final related = allTransactions.where((tx) =>
      tx.walletId == wallet.id || tx.sourceAccount == wallet.id || tx.destinationAccount == wallet.id,
      ...
      child: SafeArea(
        child: ListView(
          ...
          children: [
                ...
                children: [
                  Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(wallet.bankName, style: AppTypography.labelMedium.copyWith(fontWeight: FontWeight.w800)),
            ...
            _AiCard(monthNet: monthNet, monthIncome: monthIncome, monthExpense: monthExpense, topSpending: topSpending),
            const SizedBox(height: 14),
            Row(children: [
              Expanded(child: _MetricCard(title: 'Pemasukan', amount: monthIncome, color: AppColors.success, icon: LucideIcons.arrowDownLeft)),
              const SizedBox(width: 10),
              Expanded(child: _MetricCard(title: 'Pengeluaran', amount: monthExpense, color: AppColors.danger, icon: LucideIcons.arrowUpRight)),
            ]),
            const SizedBox(height: 10),
            _ActivityCard(monthIncome: monthIncome, monthExpense: monthExpense, transferIn: transferIn, transferOut: transferOut, net: monthNet),
            const SizedBox(height: 10),
            _TopSpendingCard(items: topSpending.take(3).toList()),
            const SizedBox(height: 18),
            Row(children: [
              Expanded(child: Text('Transaksi Terbaru', style: AppTypography.heading3.copyWith(fontWeight: FontWeight.w800))),
              Text('${related.length} transaksi', style: AppTypography.caption),
            ]),
            const SizedBox(height: 8),
            if (related.isEmpty)
              PremiumCard(padding: const EdgeInsets.all(18), child: Text('Belum ada transaksi pada wallet ini.', textAlign: TextAlign.center, style: AppTypography.bodySmall))
            else
              ...related.take(12).map((tx) => _TransactionRow(transaction: tx, walletId: wallet.id)),
            const SizedBox(height: 18),
...
class _AiCard extends StatelessWidget {
  const _AiCard({required this.monthNet, required this.monthIncome, required this.monthExpense, required this.topSpending});
  final double monthNet;
  final double monthIncome;
  final double monthExpense;
  final List<MapEntry<TransactionCategory, double>> topSpending;

  @override
  Widget build(BuildContext context) {
    final hasData = monthIncome > 0 || monthExpense > 0;
    final top = topSpending.isEmpty ? null : topSpending.first;
    final text = !hasData
        ? 'Belum cukup data untuk menemukan pola wallet ini.'
        : top == null
            ? 'Bulan ini belum ada pengeluaran tercatat.'
            : 'Pengeluaran terbesar bulan ini adalah ${_categoryLabel(top.key)} sebesar ${rupiah(top.value)}.';
    final headline = !hasData ? 'Belum ada insight' : monthNet >= 0 ? 'Cashflow bulan ini positif.' : 'Cashflow bulan ini negatif.';
    return Container(
      padding: const EdgeInsets.all(15),
      decoration: BoxDecoration(
        gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF17131F), Color(0xFF0D0D14)]),
      ...
      child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(headline, style: AppTypography.bodySmall.copyWith(fontWeight: FontWeight.w700)),
...
class _ActivityCard extends StatelessWidget {
  const _ActivityCard({required this.monthIncome, required this.monthExpense, required this.transferIn, required this.transferOut, required this.net});
  final double monthIncome;
  final double monthExpense;
  ...
  Widget build(BuildContext context) => PremiumCard(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Perubahan Bulan Ini', style: AppTypography.labelMedium.copyWith(fontWeight: FontWeight.w800)),
      const SizedBox(height: 8),
      _FlowRow(label: 'Pemasukan', amount: monthIncome, color: AppColors.success),
      _FlowRow(label: 'Pengeluaran', amount: -monthExpense, color: AppColors.danger),
      ...
      _FlowRow(label: 'Net', amount: net, color: net >= 0 ? AppColors.success : AppColors.danger),
    ]),
  );
}

class _TopSpendingCard extends StatelessWidget {
  const _TopSpendingCard({required this.items});
  final List<MapEntry<TransactionCategory, double>> items;
  ...
  Widget build(BuildContext context) => PremiumCard(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 8),
      if (items.isEmpty)
        Text('Belum ada pengeluaran bulan ini.', style: AppTypography.caption)
      else
        ...items.map((item) => _FlowRow(label: _categoryLabel(item.key), amount: item.value, color: AppColors.textPrimary)),
    ]),
  );
}
...
class _MetricCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => PremiumCard(
    ...
    child: Row(children: [
      Container(width: 32, height: 32, decoration: BoxDecoration(color: color.withValues(alpha: .12), borderRadius: AppRadius.radiusLG), child: Icon(icon, size: 16, color: color)),
...
class _FlowRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(children: [Expanded(child: Text(label, style: AppTypography.caption)), Text(rupiah(amount), style: AppTypography.amountSmall.copyWith(color: color, fontWeight: FontWeight.w800))]),
  );
}
      ...
      child: PremiumCard(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('${transaction.date.day.toString().padLeft(2, '0')}/${transaction.date.month.toString().padLeft(2, '0')}/${transaction.date.year}', style: AppTypography.caption),
          ])),
...
class _InfoRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Text(label, style: AppTypography.caption)), const SizedBox(width: 12), Flexible(child: Text(value, textAlign: TextAlign.right, style: AppTypography.amountSmall.copyWith(fontWeight: FontWeight.w700)))]),
  );
}
