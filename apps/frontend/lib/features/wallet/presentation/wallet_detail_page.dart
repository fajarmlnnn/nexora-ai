...
class WalletDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final related = transactions.where((tx) => tx.walletId == wallet.id || tx.sourceAccount == wallet.id || tx.destinationAccount == wallet.id).toList()
...
class _AiCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final insight = topSpending == null
        ...
        : 'Pengeluaran terbesar bulan ini: ${topSpending!.key.name} (${rupiah(topSpending!.value)}).';
...
class _MetricCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => PremiumCard(padding: const EdgeInsets.all(13), child: Row(children: [
    ...
    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: AppTypography.caption), const SizedBox(height: 2), Text(rupiah(amount), maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.labelMedium.copyWith(fontWeight: FontWeight.w800))])),
  ]));
}
