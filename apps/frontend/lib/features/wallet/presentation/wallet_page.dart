...
class _WalletHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Wallet', style: AppTypography.heading1), Text('Kelola semua asetmu', style: AppTypography.bodySmall)])),
        Container(width: 42, height: 42, decoration: BoxDecoration(color: AppColors.card, shape: BoxShape.circle, border: Border.all(color: AppColors.border.withValues(alpha: .45))), child: Material(color: Colors.transparent, shape: const CircleBorder(), child: InkWell(onTap: onAdd, customBorder: const CircleBorder(), child: const Center(child: Icon(LucideIcons.plus, color: AppColors.primaryLight, size: 20))))),
      ]);
}
...
class _TotalAssetsCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => PremiumCard(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [Text('Total Aset', style: AppTypography.bodySmall), const SizedBox(width: 6), GestureDetector(onTap: onToggle, child: Icon(visible ? LucideIcons.eye : LucideIcons.eyeOff, size: 17, color: AppColors.textSecondary)), const Spacer(), Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: Colors.white.withValues(alpha: .055), borderRadius: AppRadius.radiusPill, border: Border.all(color: AppColors.border.withValues(alpha: .45))), child: Text('Bulan ini', style: AppTypography.caption.copyWith(fontWeight: FontWeight.w700)))]),
          const SizedBox(height: 4),
          Row(crossAxisAlignment: CrossAxisAlignment.end, children: [Expanded(child: _StaticAmount(text: visible ? rupiah(balance) : 'Rp •••••••••', style: AppTypography.displaySmall.copyWith(fontWeight: FontWeight.w800, color: Colors.white, fontSize: 26))), const SizedBox(width: 8), const SizedBox(width: 110, height: 45, child: _AssetSparkline())]),
          const SizedBox(height: 5),
          Row(children: [Container(padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 5), decoration: BoxDecoration(color: AppColors.success.withValues(alpha: .13), borderRadius: AppRadius.radiusPill), child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(LucideIcons.trendingUp, size: 14, color: AppColors.success), const SizedBox(width: 4), Text('+4.8%', style: AppTypography.caption.copyWith(color: AppColors.success, fontWeight: FontWeight.w800))])), const SizedBox(width: 8), Text('vs bulan lalu', style: AppTypography.bodySmall)]),
        ]),
      );
}

class _StaticAmount extends StatelessWidget {
  const _StaticAmount({required this.text, required this.style});
  final String text;
  final TextStyle style;
  @override
  Widget build(BuildContext context) => FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerLeft, child: Text(text, maxLines: 1, softWrap: false, style: style));
}
...
class _SparklinePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawPath(path, glow); canvas.drawPath(path, line); canvas.drawCircle(points.last, 3.5, Paint()..color = AppColors.primaryLight);
  }
  @override
...
class _PrimaryWalletCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => InkWell(
        ...
        child: PremiumCard(
          ...
          borderRadius: AppRadius.radiusXXL,
          child: Row(children: [
            PremiumIconBadge(icon: walletTypeIcon(wallet.type), color: AppColors.primary, size: 46),
            ...
            Flexible(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('Saldo', style: AppTypography.caption), _StaticAmount(text: rupiah(wallet.balance), style: AppTypography.labelLarge.copyWith(fontWeight: FontWeight.w800)), const SizedBox(height: 2), const Icon(LucideIcons.chevronRight, size: 17, color: AppColors.primaryLight)])),
          ]),
        ),
      );
}
...
class _SummaryItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return PremiumCard(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 9), borderRadius: AppRadius.radiusXL, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [PremiumIconBadge(icon: walletTypeIcon(type), color: AppColors.primary, size: 31), const SizedBox(height: 6), Text(_typeLabel(type), maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.caption), const SizedBox(height: 1), _StaticAmount(text: rupiah(total), style: AppTypography.caption.copyWith(color: Colors.white, fontWeight: FontWeight.w800))]));
  }
  String _typeLabel(WalletType type) {
    switch (type) { case WalletType.bank: return 'Bank'; case WalletType.ewallet: return 'E-Wallet'; case WalletType.cash: return 'Cash'; case WalletType.investment: return 'Investasi'; }
  }
}
...
class _WalletListTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) => InkWell(
        ...
        child: PremiumCard(
          ...
          borderRadius: AppRadius.radiusXL,
          child: Row(children: [
            PremiumIconBadge(icon: walletTypeIcon(wallet.type), color: AppColors.primary, size: 38),
            const SizedBox(width: 9),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [Flexible(child: Text(wallet.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.labelLarge.copyWith(fontWeight: FontWeight.w700))), if (wallet.isPrimary) ...[const SizedBox(width: 5), Text('PRIMARY', style: AppTypography.caption.copyWith(color: AppColors.primaryLight, fontSize: 8, fontWeight: FontWeight.w800))]]),
              const SizedBox(height: 1),
              Text('${wallet.bankName} • ${wallet.maskedAccount}', maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.caption),
            ])),
            const SizedBox(width: 7),
            Flexible(child: Row(mainAxisAlignment: MainAxisAlignment.end, children: [Flexible(child: _StaticAmount(text: rupiah(wallet.balance), style: AppTypography.labelLarge.copyWith(fontWeight: FontWeight.w800))), const SizedBox(width: 4), const Icon(LucideIcons.chevronRight, size: 15, color: AppColors.textSecondary)])),
          ]),
        ),
      );
}
...
class _WalletAiInsight extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          gradient: const LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [Color(0xFF080910), Color(0xFF17102C), Color(0xFF32155F)]),
          borderRadius: AppRadius.radiusXXL,
        ...
        child: Row(children: [
          SizedBox(width: 78, height: 82, child: Stack(alignment: Alignment.center, children: [Container(width: 62, height: 62, decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [BoxShadow(color: AppColors.primaryLight.withValues(alpha: .24), blurRadius: 26)])), SvgPicture.asset('assets/mascot/nexora_mascot_master.svg', width: 74, height: 74)])),
          const SizedBox(width: 7),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('Wallet insight', style: AppTypography.labelLarge.copyWith(color: Colors.white, fontWeight: FontWeight.w800)),
            const SizedBox(height: 2),
            Text('Saldo dan distribusi asetmu terlihat sehat. Jaga dana operasional tetap terpisah dari dana tujuan.', maxLines: 2, overflow: TextOverflow.ellipsis, style: AppTypography.caption.copyWith(color: Colors.white.withValues(alpha: .72), height: 1.25)),
            const SizedBox(height: 5),
            Text('Lihat saran  →', style: AppTypography.caption.copyWith(color: AppColors.primaryLight, fontWeight: FontWeight.w800)),
          ])),
...
IconData walletTypeIcon(WalletType type) {
  switch (type) { case WalletType.bank: return LucideIcons.landmark; case WalletType.ewallet: return LucideIcons.walletMinimal; case WalletType.cash: return LucideIcons.banknote; case WalletType.investment: return LucideIcons.chartColumn; }
}