...
class _WalletPageState extends ConsumerState<WalletPage> {
            ...
            child: ListView(
              ...
              children: [
                ...
                for (final wallet in wallets) ...[
                  _WalletTile(wallet: wallet, totalBalance: total, onEdit: () => showEditWalletSheet(context, ref, wallet), onDelete: () => _deleteWallet(wallet)),
  ...
  Future<void> _deleteWallet(WalletModel wallet) async {
    ...
    final relatedCount = transactions.where((tx) => tx.walletId == wallet.id || tx.sourceAccount == wallet.id || tx.destinationAccount == wallet.id).length;
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: Text(relatedCount > 0 ? 'Wallet masih digunakan' : 'Hapus wallet?'),
        content: Text(relatedCount > 0
            ? 'Wallet ${wallet.name} memiliki $relatedCount transaksi. Histori transaksi tetap tersimpan, tetapi wallet akan dihapus dari daftar.'
            : 'Wallet ${wallet.name} akan dihapus dari daftar wallet.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(dialogContext, false), child: const Text('Batal')),
          FilledButton(onPressed: () => Navigator.pop(dialogContext, true), child: Text(relatedCount > 0 ? 'Tetap hapus' : 'Hapus')),
        ],
      ),
    );
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(success ? 'Wallet berhasil dihapus.' : 'Wallet gagal dihapus.')));
  }
}
...
class _WalletTile extends StatelessWidget {
  const _WalletTile({required this.wallet, required this.totalBalance, required this.onEdit, required this.onDelete});
      ...
      child: PremiumCard(
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [Flexible(child: Text(wallet.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: AppTypography.labelMedium.copyWith(fontWeight: FontWeight.w800))), if (wallet.isPrimary) ...[const SizedBox(width: 5), const Text('PRIMARY', style: TextStyle(color: AppColors.primaryLight, fontSize: 8, fontWeight: FontWeight.w800))]]),
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => const [
              PopupMenuItem(value: 'edit', child: ListTile(leading: Icon(LucideIcons.pencil), title: Text('Edit wallet'))),
              PopupMenuItem(value: 'delete', child: ListTile(leading: Icon(LucideIcons.trash2, color: AppColors.danger), title: Text('Hapus wallet', style: TextStyle(color: AppColors.danger)))),
            ],
          ),
        ]),
      ),
...
class _EmptyWallet extends StatelessWidget {
  ...
  final VoidCallback onAdd;
  @override
  Widget build(BuildContext context) => ListView(padding: AppSpacing.screen.copyWith(bottom: AppSpacing.bottomNav(context) + 20), children: [
    _Header(onAdd: onAdd), const SizedBox(height: 120),
    Icon(LucideIcons.walletMinimal, size: 58, color: AppColors.primaryLight.withValues(alpha: .8)), const SizedBox(height: 18),
    Text('Belum Ada Wallet', textAlign: TextAlign.center, style: AppTypography.heading2.copyWith(fontWeight: FontWeight.w800)), const SizedBox(height: 8),
    ...
    const SizedBox(height: 20), Center(child: _AddWalletInline(onTap: onAdd)),
  ]);
}
...
class _ErrorState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
    const Icon(LucideIcons.triangleAlert, color: AppColors.warning, size: 36), const SizedBox(height: 10),
    Text('Wallet gagal dimuat', style: AppTypography.labelLarge.copyWith(fontWeight: FontWeight.w800)), const SizedBox(height: 4),
    Text(message, textAlign: TextAlign.center, maxLines: 3, overflow: TextOverflow.ellipsis, style: AppTypography.caption), const SizedBox(height: 14),
