...
begin
  ...
  if v_key is not null then
    select * into v_existing from public.goal_contributions where user_id=v_user and idempotency_key=v_key limit 1;
    if found then
      select * into v_goal from public.goals where id=v_existing.goal_id and user_id=v_user;
  ...
  select * into v_wallet from public.wallets where id=p_wallet_id and user_id=v_user for update;
  if not found then raise exception 'Funding wallet not found'; end if;
  if v_wallet.balance-p_amount < v_wallet.minimum_balance then raise exception 'Wallet balance cannot fall below minimum balance'; end if;
  ...
  insert into public.transactions(user_id,wallet_id,type,amount,category,description,occurred_at,idempotency_key,metadata)
  values (
    v_user,
    v_wallet.id,
    'expense',
    ...
    now(),
    v_transaction_key,
    jsonb_build_object('goal_id',v_goal.id,'goal_contribution_id',v_contribution.id,'kind','goal_contribution')
  );
...
revoke all on function public.nexora_contribute_to_goal_from_wallet(uuid,uuid,numeric,text,text) from public, anon;
grant execute on function public.nexora_contribute_to_goal_from_wallet(uuid,uuid,numeric,text,text) to authenticated;

do $$
declare r record; v_wallet uuid; v_tx_key text;
begin
  for r in
    select gc.id as contribution_id,gc.goal_id,gc.user_id,gc.amount,gc.contributed_at,g.name as goal_name
    from public.goal_contributions gc
    join public.goals g on g.id=gc.goal_id
    where not exists (select 1 from public.transactions t where t.user_id=gc.user_id and t.metadata ->> 'goal_contribution_id'=gc.id::text)
    order by gc.created_at asc
  loop
    -- Existing goal contributions were created before wallet funding was wired.
    -- Reconcile them against the visible wallet that actually backs the app.
    select w.id into v_wallet
    from public.wallets w
    where w.user_id=r.user_id and not w.is_hidden
    order by w.is_primary desc,w.balance desc,w.created_at asc
    limit 1;

    if v_wallet is null then
      raise exception 'Cannot reconcile goal contribution %: user has no visible wallet',r.contribution_id;
    end if;
    ...
    values (
      ...
      r.contributed_at,
      v_tx_key,
      jsonb_build_object('goal_id',r.goal_id,'goal_contribution_id',r.contribution_id,'kind','goal_contribution_reconciliation')
    );
  end loop;
end $$;
