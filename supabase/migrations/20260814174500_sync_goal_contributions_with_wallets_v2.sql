...
begin
  ...
  -- Goal funding is represented in the financial ledger as an expense so the
  -- existing transaction trigger remains the single source of truth for the
  -- wallet balance. The goal itself remains an asset in Financial Overview.
...
begin
  ...
  loop
    ...
    -- Reconcile them against the visible wallet that backs the app.
